...
const validKeys = (p: number, q: number, e: number, f: number, d: number) => {
	if (!isPrime(p) || p <= 1) {
...
export const encryptRsa = (text: string, p: number, q: number, e: number, alphabet: string[]) => {
	try {
		const f = (p - 1) * (q - 1);
		const d = getKeyD(f, e);
		validKeys(p, q, e, f, d);
		...
		for (let el of newText) {
			result.push(Number((BigInt(el)**BigInt(e)) % (BigInt(p)*BigInt(q))));
		}
		return result.join(' ');
	} catch (e) {
...
export const decryptRsa = (text: string, p: number, q: number, e: number, alphabet: string[]) => {
	try {
		const f = (p - 1) * (q - 1);
		const d = getKeyD(f, e);
		console.log(d);
		validKeys(p, q, e, f, d);
		const newText = text.split(' ').map(el => parseInt(el));
		const result: number[] = [];
		for (let el of newText) {
			result.push(Number(BigInt(el)**(BigInt(d)) % (BigInt(p) * BigInt(q))));
		}
		console.log(result);
		return result.map(el => alphabet[Number(el - 1)]).join('');
	} catch (e) {
		if (e instanceof Error) {
			return e.message
		} else {
